...
export class Curation {
   constructor(
      ...
      this.id = id;
      this.trndyScore = trndyScore;
      this.individualityScore = individualityScore;
...
const curationRepository = {
   ...
   findList: async ({ where, skip = 0, take = 10, orderBy = { createdAt: "desc" } }) => {
      ...
      return prisma.curation.findMany({
         ...
         select: {
            id: true,
            styleId: true, // 스타일 필터링 기준

            // 요구사항에 명시된 필드
            trndyScore: true, // 트렌디 점수
            individualityScore: true, // 개성 점수
            practicalityScore: true, // 실용성 점수
